...
const UseDataLoaderPlugin = require('./plugin/useDataLoader');
...
module.exports = async (app) => {
  ...
  // 啟用ctx.useDataLoader
  options.plugins.push(UseDataLoaderPlugin());
  ...
  if (!options.schema) {
    ...
    if (!options.resolvers) {
      if (!config.resolversDir) {
        app.logger.info('\x1b[31m[apollo] %s\x1b[0m', 'WARNING: resolvers not defined.');
      } else {
        // 先去判斷config.typeDefsDir是否真的有這個目錄
        // 如果沒有，就加上app.appInfo.root去尋找
  ...
  app.logger.debug(`\x1b[34m[apollo] Server running on http://localhost:${app.config.port}${config.path}\x1b[0m`);
};
